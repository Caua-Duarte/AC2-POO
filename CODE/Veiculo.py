class Veiculo:
    ...
    #Cadastro do veiculo
    def __init__(self,tipo_veiculo,modelo_veiculo,cor_veiculo,placa_veiculo,tipo_combustivel='gc',ano_veiculo=int,potencia_motor=100):
        ...
        if self.t_v not in __tipo_veiculo.keys():
            raise KeyError('\033[1;31mTipo de veiculo não encontrado\033[m')
        else:
                ...
                if '-' in self.p_v and self.p_v.count('-') == 1:
                    for c in self.p_v:
                        if c.isalnum() == False and c != '-':
                            raise ValueError('\033[1;31mPlaca inválida\033[m')
                else:
                    raise ValueError('\033[1;31mPlaca inválida\033[m')
            else:
                raise ValueError('\033[1;31mPlaca inválida\033[m')
        ...
        if self.t_c not in __tipo_combustivel.keys():
            raise KeyError('\033[1;31mTipo de gasolina inválida\033[m')
        else:
            self.t_c = __tipo_combustivel[self.t_c]

        #Verifica se o ano do veiculo é válido
        if dt.date.today().year < self.a_v:
            raise ValueError('\033[1;31mAno do veiculo inválido\033[m')

        #Verifica se a potência do veiculo é válida
        if self.p_m <= 0:
            raise TypeError('\033[1;31mPotência inválida\033[m')
        ...
        if self.p_v not in self.veiculo_lv:
            ...
            raise print('\033[1;31mVeiculo já cadastrado\033[m')
...
if __name__ == '__main__':
    v1 = Veiculo('c','s','pt','r','ec',2019,80)
